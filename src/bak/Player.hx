...
class Player extends FlxSprite {
	...
	private function updateMovement(elapsed:Float) {
		...
		if (validCheck) {
			...
			var moveVel:Float = ((BOX_SIZE / Conductor.instance.step_ms) * lt.states.PlayState.instance.speedRate) * elapsed;
	...
	private function _handleInputs(tile_group:FlxTypedGroup<ArrowTile>) {
		...
		if (_nextTile != null) {
			...
			var curTime = Conductor.instance.time - Conductor.instance.offset;

			var _tileTime:Float = (_nextStep * Conductor.instance.step_ms);
			var _timeDiff:Float = curTime - _tileTime;

			if (!usingAuto) {
				var _hitable:Bool = _tileTime > curTime - (Conductor.instance.safe_zone_offset * 1.5)
					&& _tileTime < curTime + (Conductor.instance.safe_zone_offset * 0.4);
				FlxG.watch.addQuick("can hit", _hitable);
				if (_hitable && keyPressed) {
					trace("Yay " + _timeDiff);
					
					__tileHit(_nextTile);
				} else if (!_nextTile.missed && _tileTime < curTime - (Conductor.instance.safe_zone_offset * 0.4)) {
