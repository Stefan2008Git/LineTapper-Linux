...
import lt.objects.TileEffect;
import lt.backend.Utils.RGB;
import flixel.graphics.FlxGraphic;
import lt.backend.Conductor;
import lime.graphics.Image;
import lt.objects.Player.Direction;
...
class ArrowTile extends FlxSprite {
	 ...
	 */
	public function new(nX:Float, nY:Float, dir:Direction, time:Float) {
		super(nX, nY);
		this.time = time;
		direction = dir;
    ...
    function initProperties() {
        // Colors //
        var colorIndex = Std.int((time / Conductor.instance.step_ms) % 4);
        var colorData = [
	...
	override function update(elapsed:Float) {
		if (Conductor.instance.current_steps + 6 > time && Conductor.instance.current_steps < time) {
			var timeDiff:Float = Math.abs((Conductor.instance.time - (time * Conductor.instance.step_ms)) / (Conductor.instance.step_ms*6));	
		...
		if (Conductor.instance.current_steps + 10 > time && Conductor.instance.current_steps < time && alpha < 1) {
			alpha += 2 * elapsed;
		} else {
			alpha -= 3 * elapsed;
		}
